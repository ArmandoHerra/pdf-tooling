...
the containment *primitive* — :func:`pdf_tooling.safety.paths.ensure_within` — and
...
* Exit **5** (safety refusal) — a *substituted value* (``{stem}`` carrying
  ...
  the invocation, so it is :class:`~pdf_tooling.errors.OutputEscapesDirError`
...
from pdf_tooling.safety.paths import ensure_within
...
def render_name(
    ...
    *,
    ...
    :func:`~pdf_tooling.safety.paths.ensure_within` call. Callers pass only the
