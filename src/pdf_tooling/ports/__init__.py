...
``pdf_tooling.adapters`` and, for two of them, a capability-selected secondary.
...
returning one :class:`~pdf_tooling.models.EngineReport`, and (c) ``adapters()``
...
:class:`~pdf_tooling.errors.EngineMissingError` — exit **3**, carrying the port
name, the OS-aware install hint, and the literal string ``pdftooling doctor``
...
def _probe_for(port: str) -> EngineReport:
    ...
    from pdf_tooling.ports import compose, ocr, office, raster, structure, text
...
def require(port: str, *, capability: str | None = None) -> Adapter:
    ...
    Raises:
        EngineMissingError: Exit 3. The message always carries the install hint
            and always names ``pdftooling doctor``.
    """
    _check_port(port)
    report = resolve(port)
...
def _missing_message(report: EngineReport, capability: str | None) -> str:
    ...
    return (
        ...
        f"Run 'pdftooling doctor' to see which engines resolved."
    )
