...
method's return; only :class:`~pdf_tooling.ports.raster.RenderedPage`'s plain
data does.
        ...
        *,
        ...
        Args:
            password: PDF-37 -- the REVEALED plaintext, or ``None``. Plain
                ``str``, not :class:`~pdf_tooling.secret.Secret`: a
...
def _displayed_size(page: Any) -> tuple[float, float]:
    ...
    match :class:`~pdf_tooling.models.PageInfo`'s convention: ``PageInfo``
