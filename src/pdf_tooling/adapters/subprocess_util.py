...
# at `/dev/null/pdftooling-probe`, `soffice --version` still prints
...
#: remember to remove.
PROBE_SANDBOX_ROOT: Final[str] = os.path.join(os.devnull, "pdftooling-probe")
    ...
    *,
    ...
    Args:
        ...
        check: When true, a non-zero exit (or a timeout) raises
            :class:`~pdf_tooling.errors.FailureError` (exit code 1) carrying the
