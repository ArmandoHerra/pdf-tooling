...
to :func:`~pdf_tooling.ops.pagerange.parse`; ``str.split(",")`` for the part
...
rendered — through :mod:`pdf_tooling.safety.naming`, containment-checked —
...
:func:`~pdf_tooling.safety.atomic.plan_output_set` (never inside
:class:`~pdf_tooling.safety.atomic.AtomicWriter`'s own per-target gate,
...
from pdf_tooling.errors import NoInputError, UsageError
...
from pdf_tooling.ops.pagerange import PageRangeError, parse
...
from pdf_tooling.safety.paths import check_output_collisions, classify_operand
    ...
    *source* on every row, ``output`` the part path, ``message`` the
    ...
    try:
        with engine.open_document(source, password=resolver.for_source(source)) as document:
            ...
            plan = plan_output_set(targets, out_dir=out_dir, policy=policy, sources=[source])
