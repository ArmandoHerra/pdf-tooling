...
1. **Handing the renderer pixels instead of a path.** The passthrough is offered
   ...
   :class:`~pdf_tooling.ports.compose.ImagePlacement` carries ``raster=None`` for
...
:class:`~pdf_tooling.safety.atomic.AtomicWriter`, handing the engine
...
def _image_read_error(path: Path, error: Exception) -> PdfToolkitError:
    ...
    :func:`~pdf_tooling.safety.paths.read_source_bytes` cannot serve because none
    ...
    :func:`~pdf_tooling.safety.paths.unreadable_source_error` returns rather than
    ...
    **The accessibility question is asked first, and the module's own vocabulary
    ...
    Like :func:`~pdf_tooling.safety.paths.source_read_error`, this refines the
...
def _engine_boundary_error(placements: Sequence[ImagePlacement], error: OSError) -> PdfToolkitError:
    ...
    *cannot* belt -- it can, and two adapters demonstrably do
    (``git grep -n 'from pdf_tooling.safety' -- src/pdf_tooling/adapters``
...
def _decode_for_reencode(path: Path, mode: str) -> Image.Image:
    ...
    runs INSIDE the :class:`~pdf_tooling.safety.atomic.AtomicWriter` context, so
