...
rendered payload and nothing else, so that ``pdftooling ... | jq`` works whether
...
:class:`~pdf_tooling.secret.Secret` in ``record.msg`` or ``record.args``
...
LOGGER_NAME: Final[str] = "pdf_tooling"
...
_OWNED_MARKER: Final[str] = "_pdf_tooling_owned"
...
def _render(value: Any) -> Any:
    ...
    A :class:`~pdf_tooling.secret.Secret` is converted to ``str`` **here**,
...
def configure_logging(*, verbose: int = 0, quiet: bool = False, no_color: bool = False) -> None:
    ...
    **Two loggers, on purpose** (PDF-25 Design §D8, `d220b7d79d`). Configuring
    only ``pdf_tooling`` left the ROOT logger with zero handlers, so a record
    ...
    **Only from a CLI invocation.** This function is called from exactly one
    ...
    established as a ``pdftooling`` run. A library consumer that imports
    ``pdf_tooling`` and never calls it keeps a pristine root logger; owning
    ...
    **Only our own handlers are removed.** Root belongs to whoever is hosting
    ...
    Our own ``pdf_tooling`` logger keeps ``propagate = False``, so one record
