...
from pdf_tooling.safety.paths import classify_operand, unreadable_source_error
...
resolve, that is a broken install (exit 3). Run 'pdftooling doctor'.
"""
...
def _input_read_error(source: Path, error: OSError) -> PdfToolkitError:
    ...
    :func:`~pdf_tooling.safety.paths.read_source_bytes` -- whose own fallback
    ...
    **The accessibility question is asked first, and this verb's own vocabulary
    ...
    :func:`~pdf_tooling.safety.paths.unreadable_source_error` is the same
    predicate :func:`~pdf_tooling.safety.paths.classify_operand`'s rung 4 uses
    ...
    Unlike :func:`~pdf_tooling.safety.paths.source_read_error` this has **no
