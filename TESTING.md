...
|---|---|---|
| **Engines present** (`tesseract` + `soffice` on `PATH`) | non-zero, but **zero are engine-gated** — `scripts/assert_skips.py --expect-zero` asserts exactly that. The non-zero remainder is the pre-existing safety-spine skips (see below) plus four `test_cli_contract.py` parametrize sets that are empty until a mutating verb or a subcommand group exists (`C4`, `C9`, `C10`/`C11`/`C13`), plus the `samples`-marked arms (unset), plus **one PDF-15 skip that is NOT engine-gated** — `tests/integration/test_ocr.py::test_ac7_rotated_page_returns_the_expected_text` is unconditionally `@pytest.mark.skip`, present or absent: it is BLOCKED by a pre-existing defect in `adapters/pdfium_raster.py` (out of PDF-15's permitted edit scope; see that test's own module-level comment). |
| **Engines hidden** (`PDF_TOOLKIT_TEST_HIDE_ENGINES=tesseract,soffice`) | the engines-present count **plus at least 20 engine-gated skips** — `tests/test_doctor.py`'s existing arms (PDF-05) and `tests/test_testdata.py`'s tesseract-recovery arm (PDF-06, 7 together) **plus PDF-15's own 13** (8 in `tests/integration/test_ocr.py`, 5 in `tests/integration/test_office.py`; measured live: `PDF_TOOLKIT_TEST_HIDE_ENGINES=tesseract,soffice uv run pytest tests/integration/test_ocr.py tests/integration/test_office.py -rs -q` reports `7 passed, 14 skipped` — 13 engine-gated plus the one unconditional AC7 skip noted above). `scripts/assert_skips.py` (no `--expect-zero`) asserts this count is **non-zero**; a zero here is a regression, not vacuity, as of PDF-06. |
