...
## [PDF-42] Follow-up: the AC7 guard this spec's own comment pointed at did not exist — 2026-09-05

- **A comment in Section 6 named `tests/test_gate_budget.py::test_the_pdf42_ceilings_carry_their_measurement` as the mechanism enforcing the two new ceilings' evidence blocks, and that test was never written.** The blocks themselves were complete — date, commit, host, interpreter, engines, both arms' min/median/p95/max/spread, the factor and the plant separation — but nothing asserted they stay complete, so the constants were a comment away from being widened silently. Found by sweeping every `test_*` name referenced in this spec's own added lines against the set actually defined under `tests/`; it was the only dangling one of 29.
- **This is the exact defect class the same commit set out to fix, committed by the fix.** `PDF-42` corrected two false attributions that had lived for the whole life of Section 6 — `cli/common.py` imports `PIL`, `ops/textract.py` imports `email` — on the reasoning that *a claim about this repository is derived, gated, or absent*, and a forward reference to a test that does not exist is the same species: a hand-maintained claim in a comment, wrong, and pointing a future reader somewhere useless.
- **The guard now exists, with one observed red per required field.** `test_the_pdf42_ceilings_carry_their_measurement` parses the block adjacent to each ceiling and requires `EVIDENCE_TOKENS`, `DISTRIBUTION_TOKENS`, and — PDF-42's own addition, because "p95 times a factor" is only checkable when the factor is on the page — `FACTOR`, `SEPARATION` and both arm names. `test_a_ceiling_whose_block_loses_a_field_reddens` strips each of the eleven required fields from a scratch copy under `tmp_path` and asserts the guard notices. `ceiling_block()` is deliberately a second parser rather than a widening of `PDF-29`'s `budget_block()`, which stays pinned to its own constant.

## [PDF-42] Follow-up: this spec's own sample data reddened another spec's guard — 2026-09-05

- **`make ci` at `256e975` failed `tests/test_coverage_policy.py::test_exactly_one_test_module_scrubs_the_coverage_environment`, and the cause was sample DATA rather than behaviour.** `LOAD_SENSING_NON_CASES` listed `"COVERAGE_PROCESS_START"` as an example of an environment key the repaired load-sensing matcher must **not** flag — the most on-the-nose sample available for exactly that proposition, and the wrong one to reach for. `coverage_scrub_sites()` counts any module naming that literal as an `ast.Constant` **anywhere** under `tests/` (AST rather than grep, deliberately, so a prose mention in `test_cli_spine.py` is not miscounted), so quoting it as data made this file read as a second module scrubbing the coverage environment.
- **The guard is correct and was not touched.** Its detector, its `EXPECTED_SCRUB_SITE` and its expected count of one are unchanged, and `tests/test_cli_spine.py` remains the single sanctioned scrub site. Two ordinary environment keys carrying the same discrimination (`PYTHONPATH`, `PDF_TOOLKIT_SAMPLES_DIR`) replace it, so the matcher is still proven to separate *senses worker identity or host load* from *touches the environment* — the assertion that stops it breaking Section 6's own `dict(os.environ)` probe. No ceiling widened, no exemption lengthened, nothing marked `xfail`.

## [PDF-42] A startup control that can see latency, and an abstention guard that cannot be walked past — 2026-09-05

- **The product's headline startup claim was defended by an instrument that abstains and an instrument that measures a different proposition, and both halves are now closed.** A module-scope `time.sleep(0.5)` in `src/pdf_toolkit/cli/common.py` moves `pdftoolkit --help` from **227.0 ms to 728.7 ms** (min-of-5, quiet host, loadavg 2.18) while the attributable import census stays at **282 — a delta of exactly 0** — and all six pre-existing Section 6 tests plus the companion guard report **`7 passed`**. `-X importtime` was already handing Section 6 a self-time and a cumulative column on every line, and `_importtime_census` was keeping only the third field; this spends the two columns already on the wire, in the same subprocess pair the section already pays for. `test_no_single_module_costs_more_than_the_self_time_ceiling` now fails on the planted tree naming `pdf_toolkit.cli.common` at **506370 µs** against a 250000 µs ceiling. `STARTUP_BUDGET_MS` is untouched at **325.0**, and the wall-clock test's xdist abstention is untouched and still asserted by its own guard.
- **`MODULE_SELF_US_CEILING = 250_000` and `TOTAL_IMPORT_US_CEILING = 2_000_000` are derived, never picked**, with min/median/p95/max/spread for both the serial and the `-n auto` arm recorded beside each (N = 12 per arm, plus a second N = 12 campaign taken while a foreign suite saturated the box, because the pessimistic one is the one a ceiling has to survive). **The total-self-time net provably cannot see the 0.5 s plant and says so in terms**: the plant totals 759422 µs on a quiet host while ordinary `-n auto` contention was *observed* at 949030 µs with no plant at all — the noise exceeds the signal, so no single ceiling separates them. It ships as the coarse gross-growth ratchet it can honestly be; the per-module ceiling is the one that catches the regression, and the measurement is filed rather than tuned into a flake.
- **The claim that the replacement RUNS is now mechanized instead of argued, which is the whole difference between this item and the one it repairs.** `test_the_section_six_roster_actually_ran_under_the_default_parallelism` spawns pytest over `tests/test_import_boundaries.py` passing **no `-n` of its own** (so `pyproject.toml`'s `addopts` supplies the condition), then reads a JUnit XML and a per-worker receipt: all **13** derived Section 6 members present, none skipped, each recorded on a real xdist worker across `gw0`–`gw7`. Three reds prove each clause fails differently — a planted `skipif`, a planted empty `parametrize`, and the same subprocess forced serial. That last one **caught a real defect in this spec's own instrument**: the child inherited `PYTEST_XDIST_WORKER` from the parent worker, so a serial run reported its parent's id and would have been certified parallel. The child env now scrubs `PYTEST_XDIST_*` (and deliberately not `COVERAGE_PROCESS_*`, which `tests/test_coverage_policy.py` pins to exactly one scrubbing module).
- **Both recorded walk-pasts of the abstention guard redden.** `b5d1027aa1`: the three exact literals become a proposition over the shape of a worker-identity/host-load key, proven against eight cases including `PYTEST_XDIST_TESTRUNUID` (the filed bypass) and four siblings nobody has filed, each carrying its own assertion that the old tuple genuinely did *not* contain it — and proven **not** to fire on Section 6's legitimate `dict(os.environ)` pass-through, because a matcher that broke the section's own probe would be reverted and the hole would reopen. `d933b5abdd`: `SECTION_SIX_TESTS`'s hand-typed 3-tuple becomes a roster **derived** from `section_span()` (it read **3** while the roadmap said **5** and the file held **6**), and the collection-time silencing is caught by the receipt while the source-text check still passes on the same tree — the contrast that shows no longer list could ever have reached it.
- **Two false attributions baked into the instrument's own failure messages are corrected against measurement.** `cli/common.py` does not import `PIL` and never did (`git log -S PIL` on that path is empty); the single module-scope `PIL` site under `src/` is **`ops/compose.py:90`**, reached via `cli.main → cli.cmd_compose → ops.compose`, and it is now **derived** by `test_pil_has_exactly_one_module_scope_import_site_under_src` rather than asserted by a comment. `ops/textract.py` does not import `email` either — but `email` *is* eager, so the sentence was re-pointed at the cause actually measured with `-X importtime` (`importlib.metadata._adapters`, via `cli.cmd_version`) rather than struck or re-invented. Recorded on the way: `-X importtime` emits repeat rows (282 rows over 273 distinct names; `nt` six times), so `HELP_MODULE_CEILING`'s "280 modules" has always been a row count — noted, and changed in neither direction.

## [PDF-41] Follow-up: the `B-245` pin could never have fired its own retirement signal — 2026-09-05

- **The strict `xfail` pinning `B-245` was an unfailable criterion, so the signal the entry below promises could not have arrived.** That arm asserted the *defective* outcome (`cmd_check` returns `1`). Under the defect, `scripts/licenses.py:344` raises `KeyError` before the assertion is reached, so the arm xfails; but the day `:344` is corrected, the offender is allowlisted, `cmd_check` returns `0`, the assertion fails on its own terms — and the arm xfails **again**. Both states are `XFAIL`, the strict `xfail` never XPASSes, and the pin stays permanently green while advertising a retirement signal it could not emit. Measured, not reasoned: the arm was run against a scratch copy of `scripts/licenses.py` with `:343-344` corrected, and it still reported `1 xfailed`.
- **The fix is the assertion's polarity, and nothing else.** The arm now asserts the **correct** behaviour (`cmd_check` returns `0`). Today that assertion is never reached — the `KeyError` still fires first — so the arm still xfails and the pin still holds. When `:344` is corrected to subscript by the matched key, the assertion passes, the strict `xfail` XPASSes, and the suite goes red exactly as the pin's own reason string says it should. Both directions were verified against a mutated scratch copy under `$TMPDIR` before landing.
- **`B-245` is still filed and still NOT fixed, and `scripts/licenses.py` is still unedited.** Only the encoding of the test that pins the defect changed; the deny pattern is untouched, `ALLOWLIST` stays empty, and no guard was weakened, widened or exempted to reach green.

## [PDF-41] The `AGPL|GPL|LGPL` deny pattern and the empty `ALLOWLIST` get a standing test — 2026-09-05

- **The mechanism enforcing this product's central licensing constraint executed only as a script; now it also runs as a unit test.** `scripts/licenses.py`'s deny pattern, its `DENY_PROSE` alias, the empty `ALLOWLIST`, and the disjunctive branch that lets `pyphen` (tri-licensed `GPLv2+/LGPLv2+/MPL-1.1`) through on its clean MPL alternative were previously reached only by `make licenses` / `make licenses-check` / the `license-gate` CI job — nothing asserted their behavior. `tests/test_license_metadata_policy.py` loads the decision layer by path (never the measurement pipeline: `ensure_pinned_env`, `pip_licenses`, `rows` and `universal_names` are never invoked in their real form) and adds 34 nodes across four arms: the deny/permit table (including the three prose-only forms only `DENY_PROSE` catches, and the four MPL/`pypdfium2` rows a widened pattern would break), the disjunctive path discriminated by `permitted is True AND denied != []` (reachable only through `scripts/licenses.py:267`), the empty `ALLOWLIST`'s emptiness and its escape hatch on a static fixture, and the loud `MULTI-LICENSE (disjunctive)` report that is `B-024`'s entire mitigation.
- **Every arm was observed red before it shipped green**, per eight planted mutations run against scratch copies under `$TMPDIR` (never the working tree): a real GPL term misfiled as permitted, `DENY_PROSE` and `DENY` weakened/narrowed/emptied, the disjunctive branch flattened, an `ALLOWLIST` entry added, a fixture's allowlist entry removed, and the disjunctive banner deleted — each produced the named failure this spec predicted, and `git status --porcelain` was byte-identical before and after every run.
- **A new, previously unfiled defect (`B-245`) was found and deliberately NOT fixed here.** `scripts/licenses.py:343` tests `ALLOWLIST` membership on the PEP-503-normalized name, but `:344` subscripts `ALLOWLIST` with the raw row name — a key that normalizes equal to a row name but differs literally from it (`pdfminer-six` vs `pdfminer.six`) passes the membership guard and then raises `KeyError` instead of allowlisting. It is pinned with a strict `xfail` so the day it is corrected the suite goes red as the signal to retire the pin. `scripts/licenses.py` is unedited by this entry — filed, not fixed.
- **This is exactly one member of `B-151` (fingerprint `9385d4991e`); `B-151` itself is NOT closed by this entry** — twelve of its thirteen criteria remain uncovered, and the wider `B-151`+`B-169`+`B-180` cluster (30 criteria across five shipped specs) is deferred whole as cycle 4's theme.
- **Additive only.** `scripts/licenses.py`, `src/`, `Makefile`, `.github/workflows/ci.yml`, `uv.lock`, `pyproject.toml`, `THIRD_PARTY_LICENSES` and `website/src/data/licenses.json` are all unchanged; the deny pattern is not widened or narrowed and `ALLOWLIST` stays empty.

## [PDF-40] Follow-up: the batch arms fed `convert` the one operand kind it cannot take, and a keyword pair the secret scanner reads as a key — 2026-09-05

- **My error, and CI caught what a green local gate could not: `convert`'s operand is never a PDF.** The new batch arms built every fixture as a PDF, including for `convert`, whose own registered invocation already records that *its own operand is never a PDF*. Converting PDF→PDF exercises LibreOffice's PDF **import** filter, which needs Java: present on my host, absent on the runner, where `soffice` then failed **every** input (`failed to launch javaldx` / `source file could not be loaded`) and the arm could no longer tell the good inputs from the bad one — six `convert` cells red in `engines-present`. The fix is not a skip and not a `convert` exemption: the fixture kind is now **derived from each verb's own declared operand `metavar`** (`registry.operand_metavars()`), so `convert` gets a text document and every other verb a PDF, and a verb that changes its operand kind gets the right fixture with no author action.
- **A second self-inflicted red, and the honest fix was the code shape rather than the gate.** Splitting `compress`'s dry-run password prediction across the per-item guard put the credential keyword on the same source line as an upper-case constant, which `gitleaks`' `generic-api-key` rule reads as a key — it extracted `verb=VERB_COMPRESS` (entropy 3.84), a keyword argument naming a `Final[str]` declared and exported in the same file. The call now places one keyword per line, with the reason recorded at the call site so a later reformat cannot silently reintroduce it. Because the shape is already published in this spec's own commit and history is not rewritten on `main`, one **fingerprint-scoped** `.gitleaksignore` entry retires that single finding: it disables no rule and exempts no path, so the same shape in any new commit still fails.
- **Two remaining CI reds on that run were measured and are NOT this spec's**, and neither is claimed as fixed here: `test_render_pool_start_method.py`'s post-SIGKILL growth arm went red on `test (3.12, ubuntu-latest)` while `3.11`, `3.13` and `3.14` passed on the identical tree — the render-pool signals/teardown family, whose only line this spec touches runs after the pool has fully drained — and the earlier local gate had already surfaced the sibling stray-temp-file arm in `test_rasterize_signals.py`, which passes in isolation and is a known load-driven flake. Both are reported to the project manager rather than absorbed.
- **Forward-fix, not a rewrite.** No product behaviour changed: the continuation guard, the run/item boundary, the derived population and every acceptance arm are untouched. The diff is the fixture derivation, one call's keyword layout, and the fingerprint entry.

## [PDF-40] A `--out-dir` batch payload never denies an artifact that is on disk — 2026-09-05

- **A multi-input `--out-dir` failure now emits the OPERATION envelope where it previously emitted the ERROR envelope, and this is a behaviour change on a failure path inside the pre-`1.0.0` window.** `pdftoolkit compress a.pdf <corrupt> c.pdf --out-dir out -y` exited `1` with `out/a.pdf` **on disk** and a payload of `{"schema_version": 1, "error": {…, "path": null}}` — no collection key at all — so the input that succeeded was unreported, the input that failed was unnamed, and the batch aborted. It now reports every operand by name, in command-line order, each with its own `ok`, `exit_code` and `message`. A script that parsed such a failure by reading `payload["error"]` will find `payload["items"]` instead. **`schema_version` is unchanged at `1`**: no key is renamed, removed, retyped or repurposed, the error envelope survives untouched for single-input runs and for every run-scoped failure, and the change is by emission rather than by addition — `items`, `input`, `output`, `ok`, `exit_code` and `message` were already the batch class's own published keys, and `OperationResult.exit_code` already documented "the highest item code" as behaviour the product could not previously reach.
- **`PLAN.md` §5.4's Failure policy — a failing input is recorded, the run continues, and the run exits `1` at the end with a per-input status — is now true of the whole `--out-dir` batch class rather than of `info` alone.** The mechanism was never exception handling but seam PLACEMENT: `info` continues because its operand classification happens per item, inside the loop's own guard, while every batch verb classified per batch, outside any guard. `src/pdf_toolkit/ops/batch.py` (new) is that guard, written **once** on `operand_argument()`'s stated precedent, and the six ops modules call it rather than carrying a copy. Both failure kinds are closed and they fail at different seams: a **corrupt** input classifies clean and fails when an engine opens it, an **unreadable** input failed at a pre-flight loop that also ran duplicated in `cli/`, which is why a fix applied only under `ops/` would have left that half unfixed.
- **The run/item boundary is decided explicitly and is asserted, not merely described.** `FailureError` (exit `1`, including the unreadable and unwritable classes) and `AuthError` (exit `6`) are verdicts about one input and become rows; a usage error, a directory operand, a nonexistent input, a missing engine and the refusal family stay run-scoped and keep aborting the run with the error envelope. A missing engine fails identically for every input, so rendering it per input would have replaced one accurate diagnosis with copies of it **and changed the process's exit code** — a public exit-table change wearing a bug fix's clothes. The nonexistent-input exit `4` keeps its unconditional precedence and a directory operand is still refused before any earlier input is processed. `except Exception` is forbidden in the guard and an AST-walked test reddens on it, so the bare-exception read-seam work stays separately measurable rather than being silently swallowed here.
- **`--dry-run` did not fall behind the real run.** For the unreadable kind the preview now mirrors the real run on **both** observables — exit code and envelope shape — on every verb in the class, because operand classification runs no engine and now sits inside the guard on both paths. The corrupt kind's pre-existing preview/reality gap on `convert` (`--dry-run` exits `0`, the real run exits `1`) was measured before and after this change and is **unchanged**: closing it would require the preview to open the document, which the dry-run contract forbids. It is reported rather than closed here.
- **The population is code, not a list in a document.** `tests/registry.py::out_dir_batch_verbs()` derives the class from the live command tree in three steps — the `--out-dir` consumer set off each command's own declaration, operand arity off the live `nargs`, and the verb name the command registers rather than its module basename (`cli/cmd_office.py` registers `convert`). `split` is excluded **by arity**, so a future `split` with a variadic operand joins the class with zero author action. `tests/test_batch_continuation.py` (new) and contract row `C21` parameterize over that derivation, drive both failure kinds with the bad input **in the middle** — a leading bad input cannot tell abort from continue — and compare the payload against a filesystem listing the harness takes itself, in both directions.

## [PDF-39] Rule the pre-`v1.0.0` envelope inconsistencies, and document the mapping that ships — 2026-09-05

- **`schema_version` stays `1`, and the next increment now has a named trigger on the record.** It moves on the first change a name-keyed consumer cannot survive — a published key **renamed**, **removed**, **retyped** (including `[]` becoming `null`), or **repurposed** — and never on an addition; an increment is coupled to a major version bump and is the project manager's call. Three of those four are now mechanical: `tests/golden/envelope_keys.json` freezes, per invocable leaf, the sorted top-level `-o json` key set and the sorted key set of an `-o ndjson` line **as measured at `d03bee3` (tag `v0.2.0`), before any of this spec's edits**, and every live envelope must be a **superset** of it — superset and not equality, so addition stays free and destruction stops. The fourth, a meaning change, is stated as NOT mechanically detectable rather than pretended. The register covers **all 26 leaves with none uncovered**, measured against `discover_verbs()` rather than claimed, and regenerating it to make a red go green is forbidden (`TESTING.md`).
- **The two public permission vocabularies now have a published crossing, and neither was renamed.** `pdftoolkit info -o json` and `pdftoolkit permissions -o json` reported the same three ISO 32000-1 bits under different names — `forms`/`fill-forms`, `accessibility`/`extract-accessibility`, `print-highres`/`print-high-resolution` — so a consumer diffing the two saw three phantom differences on every encrypted document, disclosed on no surface a consumer reads. `ports/structure.py` gains one `PERMISSION_TOKEN_MAP` constant, `README.md` gains the mapping table **derived from it by a test rather than transcribed beside it**, and a behavioural arm compares the two shipped surfaces against each other rather than either against a source comment. **The `--allow` input alias was considered and REFUSED**, on the record: an accepted input token is permanent from `v1.0.0`, so it would spend an irreversible budget to close a gap a table closes reversibly — `pdftoolkit encrypt --allow fill-forms` remains a usage error and a test asserts it stays one.
- **`exit_code`, `warnings` and `duration_ms` are now on all 26 leaves.** `doctor`, `info` and `meta get` omitted all three; every other leaf carried them, `meta set` included — the population is per LEAF, and the older *"22 of 25"* framing is a top-level count that treats `meta` as atomic. `warnings` is a list and is `[]` when empty, never `null`; `exit_code` **equals the process's real exit status**, derived independently of the code the process is handed so the two can genuinely disagree and a test can see it; `duration_ms` is `0` on all three, chosen per verb with the reason recorded beside the value — `info` and `meta get` because standing byte-identity and golden arms compare independent runs of the same argv, `doctor` on `cmd_version.py`'s own precedent. `--dry-run` mirrors the real run on both observables.
- **`items` is now the universal collection key, by addition, and the design decision it reverses is recorded in the source it reverses.** `doctor -o json` and `info -o json` publish `items` alongside `ports` and `documents`, which stay primary, verbatim and first — `ports` is pinned by the plan's own `jq '.ports[]'` example and `documents` by `info`'s shipped shape, and X-410 forbids renaming either. The alias and its primary are asserted to be **the same list** so they cannot drift into meaning different things. The two docstrings and the two tests that argued for the old behaviour are corrected rather than left standing.
- **The `schema_version` injection in `output/json.py` is NOT dead code, and the record is corrected.** Traced per render path: 24 leaves supply their own value and shadow it value-identically, but `doctor` and `info` render bespoke payloads that carry none — the injected literal is their **only** source, so deleting it would have removed a published key from two published envelopes and no test would have caught it. It is kept, its docstrings are corrected, the shadowing mechanism is pinned as a test on both renderers, and no `-o ndjson` item may carry `schema_version`, `verb` or `dry_run` — a hazard with no live instance today, guarded so the first attempt is a red.

## [PDF-37] `--password-file` is honoured or refused, never accepted-and-silently-ignored — 2026-09-05

- **`SILENTLY_IGNORED` is now the empty set, asserted `== set()`, over the full derived 26-verb population.** At landing, `compress`/`delete`/`extract`/`info`/`linearize`/`merge`/`meta get`/`meta set`/`ocr`/`rasterize`/`reorder`/`repair`/`rotate`/`split`/`stamp`/`tables`/`text`/`watermark` (eighteen verbs) declared `--password-file`, shape-validated it, and then read it from nothing: `ports/structure.py`'s `open_document`/`read_document_info` took no password parameter at all, so a *correct* password produced the identical exit-6 "run 'pdftoolkit decrypt' first" message as a missing or wrong one. All eighteen now resolve the global slot through one shared seam (`ops/document_password.py`'s `PasswordResolver`, threaded through `open_document`/`read_document_info`/`read_metadata`/`write_metadata` and, for `compress`/`repair`/`linearize`, pikepdf's own `compress`/`repair`/`linearize`; for `rasterize`/`ocr`, the raster engine's `render_page`; for `text`/`tables`, the text engines' `extract_text`/`extract_lines`/`extract_tables`) and reach genuine success with the right password, a distinct "the supplied password did not unlock this document" message with the wrong one. `decrypt`/`permissions` are unchanged.
- **`--password-file` is now REFUSED, exit 2, on `compose`/`convert`/`create`/`doctor`/`encrypt`/`version` — a verb that cannot use a password says so immediately, before any document is opened**, via a new central consumption check (`cli/common.py::_check_password_file_consumption`, the same B-115 shape as the existing `--output`-flag and safety-flag checks) whose classification signal (`honours_password_file`) is derived from source — no hand-typed verb list anywhere in the decision. **This is a CONTRACT CHANGE for `encrypt`**: any invocation that previously silently accepted-and-ignored `--password-file` (exit 0, or `encrypt`'s own exit-5 already-encrypted refusal, unaffected by the flag) now exits 2 whenever that flag is given at all, correct value or not. Measured blast radius: all six REFUSED verbs move from exit 0 to exit 2 for an otherwise-valid invocation once `--password-file` is added to it.
- **The never-echo policy (X-403) is unchanged and is now exercised on all 26 verbs**: a resolved password's value never reaches a log record at any verbosity, but the resolution SOURCE and whether verification succeeded now do (`-vv`), reusing the existing `"password resolved from %s"` record and the existing `password_verified` vocabulary rather than inventing a second one. `Secret.reveal()`'s allowlist widens from one file to four (`adapters/pypdf_structure.py`, `ops/raster.py`, `ops/ocr.py`, `ops/textract.py`) — each for the same reason `adapters/pikepdf_structure.py` already needed it (a plain `str` is the one shape the underlying engine, or a `ProcessPoolExecutor` boundary, demands), never a general loosening.
- **`--dry-run` inherits the resolvability tier structurally, not by a second hand-written prediction**: for the nine verbs that already open their document unconditionally in both modes (`delete`/`extract`/`info`/`merge`/`meta get`/`reorder`/`rotate`/`split`, and `rasterize`/`ocr`/`text`/`tables`), a dry run without a resolvable password already predicted exit 6 for free; for the five whose dry run does not otherwise open anything (`compress`/`repair`/`linearize`, `watermark`/`stamp`, `meta set`), the SAME credential-free `read_encryption` check `encrypt`/`decrypt`/`permissions` already use for their own dry-run predictions is reused (`ops/document_password.predict_password_refusal`) — never resolving a value, so the correctness tier (a wrong-but-resolvable password) stays deliberately unpredicted everywhere.
- **`tests/test_password_leaks.py`'s own `HONOURED_FLOOR` grows from `{decrypt, permissions}` to the full twenty-verb measured set** (never gaining `encrypt`, per the ledger's own ruling) and `IGNORED_DEFECT_BASELINE` is now the empty frozenset; the arm that once asserted it non-empty is retired in place, with a note, rather than left green by a leftover member. `tests/test_password_file_contract.py` (new) is the derived, product-wide proof this witness is reconciled against.

## [PDF-36] No traceback, and no heap address, on the user's error path — 2026-09-04

- **Two ledger rows, one commit, and THE TWO HALVES WERE NOT MERGED — that is asserted by driving it, not by claiming it.** `6f5911ef9d`: `compress testdata/malformed.pdf` exited 1 with **0 B of stdout and 3644 B of raw traceback** (re-driven at `ae723bc`, byte-exact). `5bd9143f61`: four verbs rendered a live heap address into the message a user reads. `5bd9143f61`'s own row forbids merging them — *"fixing `6f5911ef9d` will likely create MORE of this by routing more engine exceptions into the envelope"* — so each half has its own control, on a **disjoint fixture, verb set, observable and file**. Both directions were executed against byte-restored copies (`git show ae723bc:<path>`, never a stash or a working-tree checkout): **revert the normalizer, keep the belts → AC1/AC2/AC3 green, AC4 red at 36 of 144 cells**; **revert the belts, keep the normalizer → AC4 green at 149 of 149, AC1/AC2/AC3 red**. No single mutation reddens both.
- **Half one is a MISCLASSIFICATION, and the fix is at the engine boundary — never at the terminal seam.** `cli/main.py`'s policy that an unexpected exception prints a traceback and exits 1 is correct, is untouched, and `cli/main.py` is not in the diff at all (`git diff --name-only` names no path under `src/pdf_toolkit/cli/`); `test_ac7_an_unexpected_exception_keeps_its_traceback_and_exit_1` passes unedited. What was wrong is that three post-save re-opens sat *outside* the `try` their own functions close three lines above. **The population was DERIVED by an AST walk over the module, not enumerated** (`tests/test_engine_boundary.py`) — and the walk found a **fourth** unbelted site the spec predicted three of: `pikepdf.Permissions(...)` in `encrypt`. Residue **4 → 0** over a population of **12**. The fourth site is closed by BELTING it, never by narrowing the walk. Line numbers are deliberately not pinned: adding a belt moves them, so what is pinned is the structure they were instances of.
- **Half two is fixed at ONE site, on the `B-068` precedent the product already accepted.** `errors.py::PdfToolkitError.to_dict()` is the only thing every renderer consumes, so `normalize_object_reprs()` there covers `-o table`, `-o json`, `-o ndjson` and every call site that does not exist yet — thirty `{error}` interpolations live under `src/` and a thirty-site pass is one a thirty-first undoes. **libqpdf's own sentence survives byte-identical**; a message matching no repr passes through **unchanged** (a sanitizer that silently empties a message it did not recognise is a wrong answer carrying a success exit code, and a unit control pins it). `_clean_warning` — the product's only other repr-stripper, which had **zero** tests — gains its first ever ones in the same commit.
- **The measured population is WIDER than both the ledger's and the spec's, and it is filed rather than trimmed.** The ledger names **two** leaking verbs; the spec measured **four**; with the belts in place and the normalizer reverted the drive measures **six** — `compress`, `decrypt`, `encrypt`, `linearize`, `permissions`, `repair`. The `encrypt`/`decrypt` sites the spec recorded as *"UNPROVEN, not clean"* are now **proven**, which is the ledger's own prediction confirmed empirically: belting half one routes more engine exceptions into the envelope and so creates more of half two. Both new arms (`C19`, `C20`) are registered in `POPULATIONS` with a floor, and `C20` carries a positive control on the four measured verbs because every assertion it makes is an *absence*.
- **Neither arm is written on the exit code**, which was 1 before both fixes and is 1 after them — an exit-code control would have been green *because of* the defect on every cell (`X-206` measured exactly that passing on 10 of 11 verbs of a deliberately broken binary). The envelope is frozen under `X-410`: `schema_version` stays **1**, `to_dict()`'s key set stays exactly `{code, kind, message, path}`, and `redacted`'s behaviour is proven undisturbed rather than assumed. OR-7/`X-185` was measured on this path before and after and is **recorded, not asserted equal**: the dry-run side is byte-identical across the fix on both observables, so the fix did not reach the planning tier.

## [PDF-35] Follow-up: the new fixture carried the superseded display name, and the gate that could not have caught it locally — 2026-09-04

- **My error, and the guard did its job exactly as designed.** `tests/test_brand_surfaces.py` freezes `tests/` occurrences of the superseded display name at **exactly 47** (class H). The new `test_render_pool_start_method.py` copied `setProducer(...)` verbatim from the sibling fixture in `test_rasterize_signals.py` — whose occurrence is one of the frozen 47 — so a brand-new file grew the census to 48 and **CI run `33937159831` failed all ten test legs on it**. **The fix is the string, not the count.** Re-baselining 47 → 48 would have retired the guard to make my own defect green, which is the coverage-floor gaming pattern wearing a different hat.
- **A real gate-parity gap, recorded rather than apologised for: a green local `make ci` could not have predicted this.** The census reads `git grep -Ion` **and `git show HEAD:<path>`** — i.e. **committed** state only. A new file's occurrences are structurally invisible to it until a commit containing them exists, and *not merely until `git add`*. My local gate was green (rc=0, 3067 passed) while the file was uncommitted, and green for that reason. **For any spec that ADDS a file, the honest local sequence is commit → re-run the gate → push**, and `make ci`'s own epilogue ("a green `make ci` does not predict CI") is more literally true here than it reads.
- **Forward-fix, not a rewrite.** Landed as a second commit under X-478 §12 (forward-fix-only outranks one-commit hygiene). No product code changed: the pin, the survivor matrix, and every acceptance arm are untouched — the diff is one fixture string and its explanatory comment.

## [PDF-35] Pin the render pool's start method — 2026-09-04

- **The start method stopped being an inherited premise.** `ops/procpool.py` built the render pool with no `mp_context=`, so it inherited whatever the interpreter defaulted to — `fork` on CPython 3.11–3.13, `forkserver` from 3.14, which `requires-python = ">=3.11"` and the CI matrix both make a **supported** interpreter. Under `forkserver` a worker's parent is the forkserver helper, which outlives the CLI, so `PR_SET_PDEATHSIG` is armed against the wrong process and never fires. It is now a declared product decision — `_START_METHOD = "spawn"` (private; `__all__` did not grow), ruling X-401, `forkserver` excluded on every branch. **`_set_pdeathsig_sigkill` and `_terminate_pool` were NOT touched: the pin makes them correct again untouched, and editing them would have made the green ambiguous.**
- **The red was OBSERVED before the fix, not assumed — and it is not observable by default on this host.** The project venv is CPython 3.12.13, whose default is `fork`, so the existing SIGKILL arm was green locally and proved nothing. Forced explicitly, pre-fix, with the start method as the only variable: `fork` 0 survivors / 0 growth, `spawn` 0 / 0, **`forkserver` 8 survivors and output 21 → 240 files** (the job ran to completion after the parent was reaped). End-to-end through the real CLI under a forced ambient `forkserver`: pre-fix **10 survivors, 16 → 32 files**; post-fix **0 survivors, 16 → 16**. New file `tests/integration/test_render_pool_start_method.py` keeps the matrix as a parameterized arm — **no allowlist, survivors enumerated from `/proc` by pid, non-vacuity asserted before growth** — and inverting the `forkserver` expectation fails it, which is how we know it reads the world.
- **CONTRACT change, not a behaviour change: `rasterize --help` no longer promises the `forkserver` gap.** The old text told the user, as a shipped property, that SIGKILL coverage *"does not cover the 'forkserver' start method Python 3.14 makes the Linux default"*. That is now false in the user's favour, which is still false. The corrected paragraph names the pinned method as the reason the Linux guarantee holds and **keeps the macOS gap stated** — `prctl` is Linux-only and softening that would be gaming. The existing `test_ac12_*` could not see this change **in either direction** (it pins only tokens that survive a corrected text), so a new control was added; the red was observed as a pair — the new test fails against the restored pre-fix paragraph **while `test_ac12_*` stays green on the identical tree**.
- **`deabf608c2` (`OSError: AF_UNIX path too long`) closes as a mechanical consequence, and an OR-7 divergence closed with it.** `spawn` binds no AF_UNIX listener, so the object is removed rather than the failure mitigated. Measured in the user's own terms at `$TMPDIR` length **80**: pre-fix the real CLI died in `forkserver.py::ensure_running` → `listener.bind(address)`; post-fix it completes with **no socket and no `pymp-*` directory** under `$TMPDIR`. The 32-byte suffix and the 107-character `sun_path` ceiling were re-derived against the installed stdlib and an empirical bind probe, not transcribed. Newly measured and previously unrecorded: in that pre-fix posture `--dry-run` exited **0** with a full 7-key envelope while the real run exited **1** with **no envelope at all** — an X-185 divergence on *both* observables at once. Post-fix both exit 0 with identical envelope shapes.
- **Cold start measured, published, and the "roughly one cold import, not N" shape is CONFIRMED in direction but FALSIFIED as a literal.** Penalty = `spawn` − `fork`, alternating arms, 6 pairs each, on CPython 3.12.13 / `nproc` 8, `/proc/loadavg` **0.71 / 2.52 / 8.11 before** and **1.46 / 2.57 / 8.00 after** with no dominant competing consumer — a verified-quiet window by the 1-and-5-minute gate, so these are measurements rather than indicative figures. `--threads 1`: median **+0.134 s** (spread −0.321…+0.148). Default `min(8, CPU)` = 8: median **+0.333 s** (spread +0.293…+0.426). Eight workers cost **2.5×** the one-worker penalty, not 8× — so the cost is emphatically *not* N cold imports, but neither is it *one*: concurrent cold imports contend. Both figures are far under the ~1.5 s decision threshold, so `spawn` is pinned and shipped rather than escalated. The `xfail(strict=True)` marker `PDF-21` left as this spec's control was **spent, not skipped**: `[XPASS(strict)] … 1 failed in 6.01 s` was observed post-pin on genuine CPython 3.14.4 before the marker was deleted.

## [PDF-33] Lint follow-up: `Callable` imported from `collections.abc`, and the gate that caught it — 2026-09-04

- **My error, and the local gate that would have caught it was run in the wrong order.** `tests/test_brand_surfaces.py` imported `Callable` from `typing`; ruff's `UP035` requires `collections.abc`. I ran `make test` (green, 3056 passed) before pushing but had not yet finished `make ci`, whose `lint` leg is where this lives. `make test` is not a substitute for `make ci` and this is the entry that says so out loud.
- **The consequence is the most useful thing in this entry: `PDF-34`'s new gate held, live, on a real red.** Deploy Website run `33928057494` concluded **failure** with `build` and `deploy` **never created** — the `gate (full CI)` job failed on `lint` and the publish never happened. `PDF-34` AC15 drove that half deliberately with a `workflow_dispatch`; this is the same behaviour arriving unplanned, on a `website/**` push, which is the first time the gate has blocked a publish it was actually standing in front of. The stale Open Graph card stayed deployed, which is exactly the outcome the gate exists to produce.
- **Nothing else changed.** One import line. `make ci` now passes end to end (rc=0, 472s wall clock at loadavg 9.09→10.91 on a host concurrently running CI, recorded rather than chased). The unrelated `test (3.14, macos-14)` red on run `33928057211` is `tests/integration/test_rasterize_signals.py::test_sigterm_to_parent_only_stops_new_output_and_leaves_no_survivors` leaving a stray temp file under a SIGTERM race — a signal-timing flake on macOS, untouched by this item and filed rather than chased.

## [PDF-33] The display name follows `pdf-tooling` on every surface a human reads — 2026-09-04

- **A classifier, not a sweep — and the classifier is the deliverable.** 112 occurrences of the old display name existed over 41 files (`git grep -Io | wc -l`; the `-Ioc` form silently drops `-o`, counts matching *lines* and answers **99** with a success exit code, which is recorded because a control that reports the wrong answer is worse than one that cannot fail). Exactly **31 over 19 files** moved. `tests/test_brand_surfaces.py` ships an ordered, first-match-wins rule set that assigns every occurrence one of nine roles, asserts the residual is **zero** naming `file:line`, and pins the post-state **per class, never as a repo-wide total** (`PDF-31`'s AC6 was ruled UNMEETABLE for exactly that phrasing): in-scope classes **0**, console-script alias **6**, README contract prose **1**, `tests/` **47**, `perf/` **12**, changelog a **floor** of 15. Both reds were driven: deleting a role left 47 occurrences unclassified, each named; planting the string at an unanticipated path failed naming it. The file never spells the needle as a literal — it would inflate the class-H operand it exists to measure — and `git diff --name-only -- tests/` over this commit therefore names it and nothing else.
- **The Open Graph card is pixels, and nothing here could see it drift.** `git grep -Iln "og-image"` returned five files, all documentation or configuration, none a test or a workflow. Consequence, measured **before any edit in this item**: regenerating at `6f9c4dd` with *zero* source changes already produced different bytes (committed `37bd6ebd…e079f`, regenerated `892dba79…f5ab`) — `d03bee3` changed the generator's footer and never regenerated, so a stale card shipped through the `v0.2.0` tag. `test_the_og_card_agrees_with_its_generator` closes that gap by rendering into a scratch path and byte-comparing; `generate-og-image.py` gained an optional `--out` for it, with the no-argument invocation proven byte-identical. The card now hashes `a88011cf…6ae64` and skips, never passes, on a host where no candidate font resolves.
- **The frozen boundary held, and one of its five sites had no instrument at all.** All five distribution-identifier sites are byte-unchanged and each was plant-tested: `SELF` leaked a self-entry into the inventory (`licenses-check` rc=2); `_DISTRIBUTION` produced `0.0.0+unknown`; `_TOOL_DISTRIBUTION` produced the `unknown` sentinel. **`BROKEN_INSTALL_HINT` reddened nothing** — its three apparent covers are blind by construction (two assert against hint strings their own fakes hard-code, the third compares the constant to itself), so the plant passed 139 tests. Filed as a finding and the missing arm added, cross-checking the constant against `pyproject.toml`'s `[project] name` rather than against itself. `pyproject.toml`'s own plant did **not** break the build as predicted (`uv build` rc=0 — renaming a distribution builds fine); it reddens through the licence gate instead, and the prediction is recorded as falsified.
- **`THIRD_PARTY_LICENSES` moved only by regeneration, proven by reverting.** Its two occurrences are reachable solely through `scripts/licenses.py`'s `HEADER`; `make licenses` regenerated them. The control is a revert rather than a plant: putting the old `HEADER` back and re-running returned the artifact to the old text, which a hand-edited artifact would not have done. `NOTICE` is asserted on the **built artifacts**, not on disk — read out of both the sdist and the wheel (`dist-info/licenses/NOTICE`).
- **What deliberately did not move, and three premises this item falsified.** `README.md:29` (*"Why the distribution is not …"*) stays, because swept it contradicts the line four rows above it; the six alias occurrences stay, because the alias is published and dropping it is a breaking change; `Layout.astro`'s two-word "PDF toolkit" common noun was never in the census. Falsified while executing: the four `PDF-31` absence arms are **not** at `:1751`–`:1785` (the fourth is at `:1816`, with an unrelated arm at `:1796` inside the quoted range); `docs-gate`'s arm 3 is **green** at this commit, not the expected non-zero, because `PDF-34` registered `be89f36` hours earlier — so the "failing set unchanged" criterion was re-baselined at HEAD and held empty-to-empty; and changing a `Props` interface's declared type while its destructuring default stays a string does **not** redden `astro check` (a direct annotation mismatch does, rc=1 `ts(2322)`), so the criterion's prescribed mutation is not a valid control for it.
