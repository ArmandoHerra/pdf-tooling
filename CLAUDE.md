...
3. **The structured output shapes and the exit-code table are public API.** Breaking either is a major version bump, not a patch. Add a code; never renumber one. Add a field; never rename one without touching the `to_dict()` a test pins. `schema_version` is `1` and moves only on a **rename, a removal, a type change or a meaning change** — never on an addition — and that increment is the project manager's call, not an engineer's; `README.md`'s `## Output contract` carries the ruling, the collection-key table (`items` is universal; `documents` and `ports` are frozen primaries) and both permission vocabularies' published mapping, and `tests/golden/envelope_keys.json` is the frozen key register that makes a rename or a removal fail a test. Regenerating that register to make a red go green is forbidden — see `TESTING.md`.
