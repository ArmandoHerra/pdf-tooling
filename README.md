...
The most recent sweep carrying a readable verdict is `2026-09-18_091200`, taken at commit `0283279`; `make docs-gate` re-checks this pointer on every run. This section names a sweep and a commit and never a tally — a count is wrong the day after it is written, and the ledger's own header could not hold one still for two days. Read the ledger for what is open right now.
