...
The most recent sweep carrying a readable verdict is `2026-09-19_120144`, taken at commit `2141c76`; `make docs-gate` re-checks this pointer on every run. This section names a sweep and a commit and never a tally — a count is wrong the day after it is written, and the ledger's own header could not hold one still for two days. Read the ledger for what is open right now.
